...
AnimationManager::AnimationManager(const std::string& animationPath, Model* model)
{
...
Animation* AnimationManager::getAnimation(const std::string& animationName)
{
	if (!this->_animations.contains(animationName)) throw std::exception("Animation not found");
	return &this->_animations[animationName];
}

const AssimpNodeData& AnimationManager::getRootNode() const
{
	return this->_rootNode;
}

void AnimationManager::readHierarchyData(AssimpNodeData& dest, const aiNode* src)
{
	assert(src);
